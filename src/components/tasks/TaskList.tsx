...
export default function TaskList({ tasks, canEdit }: TaskListProps) {
    ...
    return (
        <>
            <h2 className="text-5xl font-black my-5">Tareas</h2>

            <div className='flex gap-2 overflow-x-scroll lg:overflow-auto pb-32'>
                <DndContext onDragEnd={handleDragEnd}>
                    {Object.entries(groupedTasks).map(([status, tasks]) => (
                        <div key={status} className='min-w-[240px] 2xl:min-w-0 2xl:w-1/5'>
                            <h3
                                className={`capitalize text-xl font-light border-slate-300 bg-white p-2 border-t-8  ${statusStyles[status]} `}
